...
`StandingsService` (`internal/services/standings.go`) wraps a `MatchService` and exposes `GetPointsStandings`/`GetScorers`, both backing `GET /standings/points` and `GET /standings/scorers`. The actual aggregation lives in two package-level pure functions, `ComputePointsStandings`/`ComputeScorers`, which take an already-loaded `[]models.MatchWithDetails` and know nothing about how that slice was produced. This split is deliberate: today the service loads *all* matches via `MatchService.GetMatchesDetails()`, but if a future group/multi-tenant model needs scoped standings, only the data-selection step (which matches get passed in) changes — the aggregation functions stay untouched. Points standings only count matches where both teams have at least one player (an empty team means "not played yet", not a 0-0 draw); a match's result (win/draw/loss) is derived by comparing `Team.Score` between the two sides and applied to every player on each side.
...
- `Standings.vue` has two sub-tabs (Points / Top Scorers) fed directly by `GET /standings/points` and `GET /standings/scorers` — it does no aggregation itself, just renders whatever the backend returns.
