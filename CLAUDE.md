...
Out of scope so far: a multi-group selector for the rest of the app (Matches/Standings/Profile still resolve "my first group" server-side via `GetFirstGroupForPlayer`), leaving a group, removing a member, regenerating a compromised code, and roles/permissions.
