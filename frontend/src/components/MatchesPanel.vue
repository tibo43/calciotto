          ...
          <transition name="match-details">
            <div v-if="selectedMatch" class="match-details-container card-base">
              ...
              <div v-if="showTeamRoster(selectedMatch)" class="players-section">
                <!-- Sharing the composed roster is open to any member, same
                     as reading it — there's nothing privileged about who's on
                     which team, unlike the sign-up "Share on WhatsApp" link
                     in MatchDetails.vue (which invites people to sign up and
                     stays admin-only). No link, no invite code: just the
                     kick-off and the two lineups, for a WhatsApp group that
                     already has the match. -->
                <div class="teams-share-row">
                  <a :href="teamsShareUrl" target="_blank" rel="noopener"
...
import { buildTeamsShareText, buildWhatsAppShareUrl } from '@/services/whatsappShare';
...
export default {
  ...
  computed: {
    ...
    // true, so selectedMatch and its (composed) Teams are guaranteed here.
    // Kick-off (with time) for a scheduled match, the plain calendar day for
    // an unscheduled one — the same distinction formatKickoff/formatDate
    // already draw elsewhere in this file, since an unscheduled match has no
    // time-of-day to show.
    teamsShareUrl() {
      const match = this.selectedMatch;
      const dateTimeLabel = isScheduledMatch(match) ? this.formatKickoff(match) : this.formatDate(match.Date);
